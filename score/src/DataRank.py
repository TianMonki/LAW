...
class SlideWindow:
    def __init__(self, llm_tokenizer, max_workers=10, batch_size=100, window_size=8192):
    ...
    def process_batch(self, batch):
        ...
        for line in batch:
            ...
            # If the line is empty after stripping, skip to the next one
            if not json_str:
                continue
            try:
                # The entire line is now treated as the JSON string
                json_data = json.loads(json_str)
            except json.JSONDecodeError as e:
                # This will catch any lines that are not valid JSON
...
class DateSorted:
    """
    sort & deduplicate data by the inference result using Rank Aggregation
    """
    def __init__(self, threshold=0.05, workers=128, num_perm=2048, inference_paths=None, folder_path=None, file_path=None, tokenizer_path=None, output_path=None):
        self.threshold = float(threshold)
        self.workers = int(workers)
        self.num_perm = int(num_perm)
        self.inference_paths = inference_paths # Now a list of paths
        self.folder_path = folder_path
        self.file_path = file_path
        self.output_path = output_path
        if not isinstance(self.inference_paths, list) or not self.inference_paths:
            raise ValueError("inference_paths must be a non-empty list.")
    ...
    def _get_single_ranking(self, inference_path):
        '''
        Generates a single ranked list of data_ids from one inference file.
        Returns the full ranking.
        '''
        score = []
        proportions = []
        variances = []
        with jsonlines.open(inference_path) as reader:
            for obj in reader:
                data_id = obj['data_id']
        ...
        # Return the full list of data_ids, ordered by score
        return [item['data_id'] for item in score_sorted]

    def get_aggregated_ranking(self):
        """
        Aggregates rankings from multiple inference files using Borda Count.
        """
        all_rankings = []
        print("Generating individual rankings from each inference file...")
        for path in tqdm(self.inference_paths, desc="Processing inference files"):
            all_rankings.append(self._get_single_ranking(path))

        borda_scores = defaultdict(int)
        print("Aggregating ranks using Borda Count...")
        for ranking in tqdm(all_rankings, desc="Applying Borda Count"):
            num_items = len(ranking)
            for i, data_id in enumerate(ranking):
                # Score is num_items - 1 for rank 1, num_items - 2 for rank 2, etc.
                score = num_items - 1 - i
                borda_scores[data_id] += score
        
        # Sort data_ids based on their aggregated Borda score
        sorted_aggregated_ids = sorted(borda_scores.keys(), key=lambda data_id: borda_scores[data_id], reverse=True)
        
        return sorted_aggregated_ids

    def write_to_file(self):
        # Get the final aggregated and sorted list of data_ids
        final_ranked_ids = self.get_aggregated_ranking()
        
        # Determine the number of items to keep (top half)
        length = len(final_ranked_ids)
        half = length // 2
        
        # Create a set for efficient lookup
        ids_to_keep = set(final_ranked_ids[:half])
        
        cnt = 0
        print(f"Aggregated ranking complete. Keeping top {len(ids_to_keep)} items.")
        print(f"Writing filtered data to {self.output_path}...")
        
        with jsonlines.open(self.file_path, 'r') as reader, jsonlines.open(self.output_path, 'w') as writer:
            for data in tqdm(reader, desc="Filtering original data"):
                if data['data_id'] in ids_to_keep:
                    writer.write(data)
                else:
                    cnt += 1
        
        print(f"Filtered out {cnt} items.")
        print("All data has been written to the output file.")